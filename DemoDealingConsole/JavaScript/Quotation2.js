...
function FlexQuotationInit() {
    with (vsflexQuotation) {
        ...
        ColComboList(columnIndex) = "One Price|BidAsk";
        ...
        ColComboList(columnIndex) = "Ask|Bid|(Bid+Ask)/2";
        columnIndex = ColIndex(instrumentColKey.AllowedSpotTradeOrderSides);
        ColComboList(columnIndex) = "None|Buy|Sell|All";
        ...
        ColComboList(columnIndex) = "Disallow add New|Allow add new Buy only|Allow add new Sell only|Allow add new";
        //ColDataType(columnIndex) = flexDTBoolean;
...
function SuspendOrResumeForInstrument(row, col) {
    ...
    var isResume = resumeOrSuspend.toLowerCase() == "resume";
    ...
    vsflexQuotation.TextMatrix(row, col) = isResume ? "Suspend" : "Resume";
}
...
function AllowLMT(row, col) {
    ...
    var allow = allowOrReject.toLowerCase() == "allow";
    ...
    if (needChangeDirection) {
        ...
        vsflexQuotation.TextMatrix(row, col) = allow ? "Reject" : "Allow";
    }
}
	...
	if(data && data.object)
	{
		...
		switch(vsflexQuotation.ColKey(col)) {
            ...
            case instrumentColKey.IsOriginHiLo:
				...
				break;
			case instrumentColKey.PriceType:
				quotePolicyDetail.priceTypeTemp = PriceType.GetPriceTypeValue( vsflexQuotation.TextMatrix(row, col) );
...
//	{
        ...
        vsflexGrid.TextMatrix(line, _QuotationGridColIndexs.ResumeOrSuspend) = "Resume";
        vsflexGrid.Cell(flexcpForeColor, line, _QuotationGridColIndexs.ResumeOrSuspend) = color_green;

        var allowLMT = instrument.allowLMT();
        vsflexGrid.TextMatrix(line, _QuotationGridColIndexs.AllowOrRejectLMT) = allowLMT ? "Disallow" : "Allow";
...
function UpdateProperties(line, isInit) {
    ...
    if (data && data.object) {
        ...
        var allowLMT = instrument.allowLMT();
        if (isInit) {
            vsflexGrid.TextMatrix(line, _QuotationGridColIndexs.OriginType) = OriginType.GetOriginTypeString(instrument.originType);
